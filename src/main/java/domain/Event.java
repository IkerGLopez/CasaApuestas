...
public class Event implements Serializable {
	...
	public boolean emaitzaIpini(String question, String result) {
		for (Question q : questions) {
			if (q.getQuestion().equalsIgnoreCase(question))
				if (q.getResult() == null) q.setResult(result);
				else return false;
		}
		return true;
	}

	public Question getConcreteQuestion(String question) {
		for(Question q : this.getQuestions()) {
			if(q.getQuestion().equalsIgnoreCase(question)) return q;
		}
		return null;
	}
